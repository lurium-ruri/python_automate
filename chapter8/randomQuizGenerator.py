#! python3
# randomQuizeGenerator.py - ランダムに問題と答えを並べ問題集と回答集を作る

import random

BASE_PATH = './chapter8/qa/'
...
def main():
    ...
    for quiz_num in range(35):
        # 問題と答えのファイルを作成する
        quiz_file = open(f'{BASE_PATH}capitalsquiz{quiz_num + 1}.txt', 'w')
        answer_key_file = open(
            f'{BASE_PATH}capitalsquiz_answers{quiz_num + 1}.txt', 'w')

        # 問題のヘッダを書く
        quiz_file.write('名前:\n\n日付:\n\n')
        quiz_file.write(
            (' ' * 20) + f'都道府県庁所在地クイズ (問題番号 {quiz_num + 1})')
        quiz_file.write('\n\n')
